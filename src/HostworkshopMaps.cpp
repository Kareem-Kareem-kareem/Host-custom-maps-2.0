...
void HostWorkshopMaps::onLoad() {
    ...
    // FIX #1: params[0] is the command name itself in BakkesMod.
    //         The user's argument "0" is in params[1].
    //         Old check:  params.empty()   → never true, always has the cmd name
    //         Old index:  params[0]        → std::stoi("hwm_load") → CRASH
    cvarManager->registerNotifier("hwm_load",
        [this](std::vector<std::string> params) {
            ...
            // FIX #2: wrap stoi — if user types "hwm_load abc" don't crash
            try {
                LoadMap(std::stoi(params[1]), false);
            } catch (const std::invalid_argument&) {
                cvarManager->log("hwm_load: <index> must be a number");
            } catch (const std::out_of_range&) {
                cvarManager->log("hwm_load: <index> is out of range");
            }
        }, "Load Solo", PERMISSION_ALL);

    // FIX #1 (same) for hwm_lan
...
void HostWorkshopMaps::ScanMaps() {
    ...
    // FIX #3: was "a.displayName = b.displayName" (assignment, UB inside std::sort)
    //         must be < for strict-weak-ordering
...
void HostWorkshopMaps::LoadMap(int index, bool isLAN) {
    ...
    try {
        if (isLAN) {
            ...
            // FIX #4: servertravel takes a package/map name, NOT a full filesystem path.
            //         Use displayName (filename without extension) instead of fullPath.
            SetStatus("LAN travel: " + m.displayName);
            gameWrapper->ExecuteUnrealCommand("servertravel " + m.displayName);
        } else {
            // FIX #5: "load_workshop" is not a valid command.
            //         Use "open" which is the actual UE3/Rocket League map load command.
