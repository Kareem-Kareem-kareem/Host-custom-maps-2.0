...
// ═══════════════════════════════════════════════════════════════════════════
//  onLoad - Simplified to prevent crash on injection
// ═══════════════════════════════════════════════════════════════════════════
void HostWorkshopMaps::onLoad()
{
    if (!cvarManager || !gameWrapper) {
        return;
    }
    ...
    cvarManager->registerCvar("hwm_maps_directory", defPath,
        ...
        .addOnValueChanged([this](std::string oldValue, CVarWrapper cv) {
            if (this) {
    ...
    // Very safe delayed initialization
    gameWrapper->SetTimeout([this](GameWrapper*) {
        if (this && autoScanOnOpen_) ScanMaps();
...
void HostWorkshopMaps::ScanMaps()
{
    ...
    if (mapsDirectory_.empty()) {
        SetStatus("Maps directory not set");
        return;
    }
        ...
        if (gameWrapper && gameWrapper->IsInGame()) {
            ...
            if (!server.IsNull() && server.HasAuthority()) {
                ...
                if (remote > 0) {
                    ...
                    transportCountdown_ = 90; // ~1.5 seconds
                    return;
                }
            }
        }
...
void HostWorkshopMaps::TeleportLANPlayers(const std::string& mapPath)
{
    try {
        ...
        SetStatus("ServerTravel executed for " + MapNameFromPath(mapPath));
    } catch (...) {
...
void HostWorkshopMaps::Render()
{
    ...
    if (filtered.empty()) {
        ...
        ImGui::TextDisabled(mapList_.empty() ? "No maps found. Set directory and click Scan." : "No maps match search.");
    } else {
        ...
        try {
            ...
            if (!server.IsNull() && server.HasAuthority()) {
                ...
                if (remote > 0)
                    ...
                    ImGui::TextColored(ImVec4(0.6f,0.6f,0.6f,1), "Hosting (no guests)");
            }
        } catch (...) {}
    }
